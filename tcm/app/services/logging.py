...
class EventLogger:
    def __init__(self, db_path: Path, encrypted_fields: Iterable[str], fernet_key: Optional[str]) -> None:
    ...
    # ------------------------------------------------------------------
    def list_events(self, limit: int = 100, offset: int = 0, event_type: Optional[str] = None) -> List[EventRecord]:
    ...
    def export_jsonl(self, chunk_size: int = 500) -> Iterator[str]:
        offset = 0
        while True:
            batch = self.list_events(limit=chunk_size, offset=offset)
            if not batch:
                break
            for record in batch:
                yield json.dumps(
